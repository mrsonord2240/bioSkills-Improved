...
|---|---|---|
| Standard DDA, clean FDR, scriptable | Comet or Sage + Percolator/mokapot at q <= 0.01 (`references/cli_route.md`) | well-validated; rescoring boosts IDs at fixed FDR |
...
| File | Read when |
|---|---|
| `references/cli_route.md` | running Sage, Comet or MS-GF+ from the command line; rescoring the `.pin` with Percolator or mokapot; pooling runs (`examples/dda_search.sh` runs it end to end) |
